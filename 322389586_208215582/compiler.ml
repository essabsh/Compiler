...
module Reader : READER = struct
  ...
  and nt_vector str = 
    let start = word "#(" in 
    let close = (char ')') in
    let nt1 = caten start (caten nt_skip_star close) in
    let nt1 = pack nt1 (fun _-> ScmVector []) in  
    let nt2 = caten start (caten (star nt_sexpr) close) in
    let nt2 = pack nt2 (fun(_,(sexp,_)) -> ScmVector sexp) in
  ...
  and nt_list str =
    ...
    let nt3' = pack nt3' (fun (_, (sexpr, _)) -> sexpr) in
    let nt3 = disj nt3 nt3' in
    let nt2' = caten nt2' nt3 in
    let nt2' = pack nt2'
                 (fun (sexprs, sexpr) ->
                   List.fold_right
                     (fun sexpr_1 sexpr_2 -> ScmPair (sexpr_1, sexpr_2))
                     sexprs
                     sexpr) in
    let nt2 = disj nt2 nt2' in
    let nt1 = caten nt1 nt2 in
...
type expr =
  ...
  | ScmLambda of string list * lambda_kind * expr    (*(lambda (x) ()) 6 *)
...
module Tag_Parser (* : TAG_PARSER *) = struct
  ...
  let rec macro_expand_qq = function
    ...
    | (ScmSymbol _) as sexpr ->
       ScmPair (ScmSymbol "quote", ScmPair (sexpr, ScmNil))           (*  a='(a,())  *)
    | ScmPair (ScmSymbol "unquote", ScmPair (sexpr, ScmNil)) -> sexpr   (* ,(a,()) = a *)
    | ScmPair (ScmPair (ScmSymbol "unquote",
                        ScmPair (car, ScmNil)),
               cdr) ->                                                    (* '( a ($(b c)) d r g)*)
       let cdr = macro_expand_qq cdr in
       ScmPair (ScmSymbol "cons", ScmPair (car, ScmPair (cdr, ScmNil)))    (* (unqoute((a,())),cdr) => (a,cdr::())*)
  ...
  let rec macro_expand_cond_ribs ribs = 
    match ribs with 
    | ScmNil -> ScmVoid
    | ScmPair(ScmPair(expression,ScmPair(ScmSymbol "=>",ScmPair( exprf ,ScmNil))),other_ribs)->
       let expaned_ribs = macro_expand_cond_ribs other_ribs in
       ScmPair(ScmSymbol "let",
               ScmPair( ScmPair( ScmPair( ScmSymbol "value",
                                          ScmPair(expression,ScmNil)), ScmPair(
                                                                           ScmPair( ScmSymbol "f", 
                                                                          ...
                                                                          ,ScmPair(ScmPair(ScmSymbol "rest",ScmPair(
                                                                                                                ScmPair(ScmSymbol "lambda" , 
                                                                                                                        ScmPair(ScmNil , ScmPair( expaned_ribs ,
                                                                                                                                                  ScmNil
                                                                                                                                           )
                                                                                                                          )   
                                                                                                                  ),
                        ...
                        ScmPair(ScmPair( ScmSymbol "if" , 
                               ...
                               ,ScmNil)))
    | ScmPair(ScmPair (ScmSymbol "else", expressions), other_ribs) ->
       ScmPair(ScmSymbol "begin",expressions)
    | ScmPair(ScmPair (check,expressions), other_ribs) ->
       let expaned_ribs = macro_expand_cond_ribs other_ribs in
       ScmPair(ScmSymbol "if",ScmPair(check , 
                                      ScmPair ( ScmPair (ScmSymbol "begin", expressions),
                                                ScmPair (expaned_ribs ,ScmNil))))
    | _ -> raise (X_syntax "not cond exp");; 
  
  
  (*   
       | ScmNil -> ScmVoid
       | ScmPair(ScmPair (check,expressions), other_ribs) ->
       let other_ribs = macro_expand_cond_ribs other_ribs in
       ScmPair(ScmSymbol "if",ScmPair(check , 
       ScmPair ( ScmPair (ScmSymbol "begin", expressions),
       ScmPair (other_ribs ,ScmNil))))
       | ScmPair(ScmPair (ScmSymbol "else", expressions), other_ribs) ->
       ScmPair(ScmSymbol "begin",expressions)
       | ScmPair(ScmPair(expression,ScmPair(ScmSymbol "=>",ScmPair( exprf ,ScmNil))),other_ribs)->
       let other_ribs = macro_expand_cond_ribs other_ribs in 
       ScmPair (ScmSymbol "let"), ScmPair (ScmPair(ScmPair(ScmSymbol "value"),ScmPair(expression,ScmNil)),
       (ScmPair (ScmSymbol "f"), ScmPair(ScmSymbol "lambda"), ScmPair(ScmNil,ScmPair(exprf,ScmNil)))),
       (ScmPair (ScmSymbol "rest"), ScmPair((ScmSymbol "lambda"),ScmPair(ScmNil,ScmPair(ScmPair((ScmSymbol "continue"),other_ribs),ScmNil))),ScmNil)
       ScmPair((ScmSymbol "if")ScmPair(ScmPair((ScmSymbol "value"),ScmPair(ScmPair((ScmSymbol "f"),ScmNil),(ScmPair(ScmSymbol "value"),ScmNil)))),
       ScmPair("rest",ScmNil))
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       (match (scheme_list_to_ocaml ribs) with
        | ribs, ScmNil ->
           let check_ribs = fun rib -> match rib with 
                                    |(ScmPair(var, ScmPair(real_val, ScmNil))) -> (var, real_val) 
                                    | _ -> raise (X_syntax "Malformed letrec ribs") in
           let checked_ribs = List.map (check_ribs) ribs in
           let tmp_val = ScmPair(ScmSymbol"quote",
                                 ScmPair(ScmSymbol"whatever", ScmNil)) in
           let letrec_args = List.map (fun (var, _) -> 
                                 ScmPair(var, ScmPair(tmp_val, ScmNil))) checked_ribs  in
           let letrec_body = List.fold_right
                               (fun (var, real_val) exprsns ->
                                 ScmPair(ScmPair(ScmSymbol("set!"), ScmPair(var, ScmPair(real_val, ScmNil))), exprsns)) checked_ribs exprs in
           let letrec_final_exp = ScmPair(ScmSymbol("let"), 
                                          ScmPair(scheme_sexpr_list_of_sexpr_list letrec_args, letrec_body)) in
           tag_parse(letrec_final_exp)
        | _ -> raise (X_syntax "Malformed letrec syntax"))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  (*
    ...
    | ScmConst' of sexpr V
    | ScmVarGet' of var' V
    | ScmIf' of expr' * expr' * expr' V
    | ScmSeq' of expr' list V
    | ScmOr' of expr' list V
    | ScmVarSet' of var' * expr' V
    | ScmVarDef' of var' * expr' V
    | ScmBox' of var' 
    | ScmBoxGet' of var'
    | ScmBoxSet' of var' * expr'
    | ScmLambda' of string list * lambda_kind * expr' V
    | ScmApplic' of expr' * expr' list * app_kind;;   
   *)
  ...
  let annotate_lexical_address =
    let rec exec pe params env =
      match pe with 
      | ScmConst sexpr -> ScmConst' sexpr
      | ScmVarGet (Var v) -> ScmVarGet' (tag_lexical_address_for_var v params env)
      | ScmIf (test, if_then, if_else) -> ScmIf' ((exec test params env), (exec if_then params env), (exec if_else params env))
      | ScmSeq exp_list -> ScmSeq' (List.map(fun exp -> exec exp params env) exp_list)
      | ScmOr exp_list -> ScmOr' (List.map(fun exp -> exec exp params env) exp_list)
      | ScmVarSet (Var v, expr) -> ScmVarSet' ((tag_lexical_address_for_var v params env), (exec expr params env))
      | ScmVarDef (Var v, expr) -> ScmVarDef' ((Var' (v, Free)), (exec expr params env)) (*????????????*)
      | ScmLambda (lam_params, lambda_kind , exp) -> (match lambda_kind with
                                                     |Simple -> 
                                                       ScmLambda'(lam_params, Simple, (exec exp lam_params (params :: env)))
                                                     |Opt optionals -> 
                                                       ScmLambda'(lam_params, Opt optionals , (exec exp (lam_params @ [optionals]) (params :: env) ))
                                                    )
  
      | ScmApplic (app_exp, app_params) -> ScmApplic'((exec app_exp params env), (List.map(fun param -> (exec param params env)) app_params), Non_Tail_Call) 
                                                    (* Here we added app_kind even though it gets overroaded - we can assume that it's Non-Tail*)
  ...
  let annotate_tail_calls = 
    let rec exec pe is_tail_position = 
      match pe with
      | ScmConst' _ as tagged_exp -> tagged_exp 
      | ScmVarGet' _ as tagged_exp -> tagged_exp
      | ScmIf' (test , if_test , if_not_test) ->
         ScmIf'((exec test false),
                (exec if_test is_tail_position),
                (exec if_not_test is_tail_position))
      | ScmSeq' [] -> ScmSeq' []
      | ScmSeq' exp_list ->
         ScmSeq'(List.mapi
                   (fun i exp ->
                     if (i == ((List.length exp_list)-1)) 
                     then exec exp is_tail_position 
                     else exec exp false) exp_list)
      | ScmOr' [] -> ScmOr' [] 
      | ScmOr' exp_list ->
         ScmOr' (List.mapi
                   (fun i exp ->
      ...
      | ScmVarSet' (tagged_var , tagged_exp) -> ScmVarSet' (tagged_var , exec tagged_exp false) 
      | ScmVarDef' (tagged_var , tagged_exp) -> ScmVarDef' (tagged_var , exec tagged_exp false)                                                              
      | ScmBox' _ as tagged_exp -> tagged_exp
      | ScmBoxGet' _ as tagged_exp -> tagged_exp
      | ScmBoxSet' (tagged_var , tagged_exp) -> ScmBoxSet' (tagged_var , exec tagged_exp false)
      | ScmLambda' (params , lambda_kind , tagged_exp) -> (match lambda_kind with
                                                          |Simple -> 
                                                            ScmLambda'(params, Simple , exec tagged_exp true)
                                                          |Opt optionals ->
                                                            ScmLambda' (params, Opt optionals, exec tagged_exp true)
                                                         )
      | ScmApplic' (app_exp , app_params , app_kind) -> if is_tail_position 
                                                       then ScmApplic' (exec app_exp false,
                                                                        List.map (fun param -> exec param false) app_params
                                                                        ,Tail_Call)
                                                       else ScmApplic' (exec app_exp false,
                                                                        List.map(fun param -> exec param false) app_params
                                                                        ,Non_Tail_Call)
    in
    fun tagged_exp -> exec tagged_exp false;; 
...
module Code_Generation : CODE_GENERATION = struct
  ...
  let collect_free_vars pe =
    let rec run = function 
      ...
      | ScmSeq' exps' -> runs exps'
      | ScmOr' exps' -> runs exps'
      | ScmVarSet' (Var' (v,Free) , exp') -> [v] @ (run exp')
      | ScmVarSet' (_,exp') -> run exp'
      | ScmVarDef' (Var' (v,Free) , exp') -> [v] @ (run exp')
      | ScmVarDef' (_, exp') -> run exp'
      | ScmBox' (Var' (v,Free)) -> [v]
      | ScmBox' _ -> []
      | ScmBoxGet' (Var' (v,Free)) -> [v]
      | ScmBoxGet' _ -> []
      | ScmBoxSet' (Var' (v,Free) , exp') -> [v] @ (run exp')
      | ScmBoxSet' (_, exp') -> run exp'
      | ScmLambda' (_,_,exp') -> run exp'
      | ScmApplic' (exp',exps',_) -> (run exp') @ (runs exps')  
    and runs exps' =
      List.fold_left (fun frees exp' -> frees @ (run exp')) [] exps'
    in
    let consts =
      (List.map
         (fun (scm_name, _) -> scm_name) global_bindings_table) 
    and free_vars = runs pe in
    remove_duplicates
      (consts @ free_vars);;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmVarSet' (Var' (v, Free), expr') -> 
         (* 1. eval (run) expr'
            2. find v in free_var_table
            3. set the eval_run_val in v
          *)
         let eval_run_val = run params env expr' in
         let v_label = search_free_var_table v free_vars in 
         eval_run_val 
      ...
      | ScmVarSet' (Var' (v, Param minor), ScmBox' _) ->
         (Printf.sprintf "\tmov rdi, (8*1)\n")
         ^ (Printf.sprintf "\tcall malloc\n")
         ^ (Printf.sprintf "\tmov rdi, PARAM(%d)\n" minor)
         ^ (Printf.sprintf "\tmov qword [rax], rdi\n")
      ...
      | ScmBoxGet' var' ->
         ...
         ^ "\tmov rax, qword [rax]\n"
      | ScmBoxSet' (var', expr') ->
         let box = (run params env (ScmVarGet' var')) in 
         let newValue = (run params env expr') in
         newValue
         ^ "\tpush rax\n"
         ^ box
         ^ "\tpop qword [rax]\n"
      ...
      | ScmLambda' (params', Opt opt, body) ->
         let label_loop_env = make_lambda_opt_loop ()
         ...
         and label_loop_params3 = make_lambda_opt_loop_params ()
         and label_loop_params_end3 = make_lambda_opt_loop_params_end ()
         in                                            
         (Printf.sprintf "\tmov rdi, (1 + 8 + 8)\n")
         ^ (Printf.sprintf "\tcall malloc\n")
         ...
         ^ (Printf.sprintf "\tmov rdi, 8 * %d\n" (env+1))
         ^ (Printf.sprintf "\tcall malloc\n")
         ^ (Printf.sprintf "\tmov rdi, ENV\n")
         ^ (Printf.sprintf "\tmov rsi, 0\n")
         ^ (Printf.sprintf "\tmov rdx, 1\n")
         ^ (Printf.sprintf "\t%s:\n" label_loop_env)
         ^ (Printf.sprintf "\tcmp rsi, %d\n" (env))
         ^ (Printf.sprintf "\tje %s\n" label_loop_env_end)
         ^ (Printf.sprintf "\tmov rcx, qword[rdi + 8*rsi]\n")
         ^ (Printf.sprintf "\tmov qword [rax + 8*rdx], rcx\n")
         ^ (Printf.sprintf "\tadd rsi, 1\n")
         ^ (Printf.sprintf "\tadd rdx, 1\n")
         ^ (Printf.sprintf "\tjmp %s\n" label_loop_env)
         ^ (Printf.sprintf "\t%s:\n" label_loop_env_end)
         ^ (Printf.sprintf "\tpop rbx\n")
         ^ (Printf.sprintf "\tmov rsi, 0\n")
         ^ (Printf.sprintf "\t%s:\n" label_loop_params)
         ^ (Printf.sprintf "\tcmp rsi, %d\n" params)
         ^ (Printf.sprintf "\tje %s\n" label_loop_params_end)
         ^ (Printf.sprintf "\tmov rdx, qword [rbp + 8*rsi + 8*4]\n")
         ^ (Printf.sprintf "\tmov qword[rbx + 8*rsi], rdx\n")
         ^ (Printf.sprintf "\tadd rsi, 1\n")
         ^ (Printf.sprintf "\tjmp %s\n" label_loop_params)
         ^ (Printf.sprintf "\t%s:\n" label_loop_params_end)
         ^ (Printf.sprintf "\tmov qword[rax], rbx\n")
         ^ (Printf.sprintf "\tmov rbx, rax\n")
         ^ (Printf.sprintf "\tpop rax\n")
         ^ (Printf.sprintf "\tmov byte[rax],T_closure\n")
         ^ (Printf.sprintf "\tmov SOB_CLOSURE_ENV(rax), rbx\n")
         ^ (Printf.sprintf "\tmov SOB_CLOSURE_CODE(rax), %s\n" label_code)
         ^ (Printf.sprintf "\tjmp %s\n" label_end)
         ^ (Printf.sprintf "\t%s:\n" label_code)
         ^ (Printf.sprintf "\tmov r15, qword[rsp + 8*2]\n")
         ...
         ^ (Printf.sprintf "\tmov rbx, rsp\n")
         ^ (Printf.sprintf "\t%s:\n" label_loop_params2)
         ^ (Printf.sprintf "\tmov rcx, qword[rbx + 8]\n")
         ^ (Printf.sprintf "\tmov qword[rbx], rcx\n")
         ^ (Printf.sprintf "\tadd rbx, 8\n")
         ^ (Printf.sprintf "\tsub rdx, 1\n")
         ^ (Printf.sprintf "\tcmp rdx, 0\n")
         ^ (Printf.sprintf "\tjne %s\n" label_loop_params2)
         ^ (Printf.sprintf "\t%s:\n" label_loop_params_end2)
         ^ (Printf.sprintf "\tadd r15, 1\n")
         ^ (Printf.sprintf "\tmov qword[rsp + 8*2], r15\n")
         ^ (Printf.sprintf "\tadd r15, 2\n")
         ^ (Printf.sprintf "\tmov qword[rsp + 8*r15], sob_nil\n")
         ^ (Printf.sprintf "\tjmp %s\n" label_stack_ok)
         ^ (Printf.sprintf "\t%s:\n" label_arity_more)
         ^ (Printf.sprintf "\tmov r9, qword[rsp + 2*8]\n")
         ^ (Printf.sprintf "\tmov rax, sob_nil\n")
         ^ (Printf.sprintf "\tmov r15, qword[rsp + 2*8]\n")
         ^ (Printf.sprintf "\tlea rbx, [rsp + 8*2 + 8*r15]\n")
         ^ (Printf.sprintf "\tsub r15, %d\n" (List.length params'))
         ^ (Printf.sprintf "\t%s:\n" label_loop)
         ^ (Printf.sprintf "\tmov rcx, rax\n")
         ^ (Printf.sprintf "\tmov rdx, qword[rbx]\n")
         ^ (Printf.sprintf "\tmov rdi, (8 * 2 + 1)\n")
         ^ (Printf.sprintf "\tcall malloc\n")
         ^ (Printf.sprintf "\tmov byte[rax], T_pair\n")
         ^ (Printf.sprintf "\tmov SOB_PAIR_CAR(rax), rdx\n")
         ^ (Printf.sprintf "\tmov SOB_PAIR_CDR(rax), rcx\n")
         ^ (Printf.sprintf "\tsub rbx, 8\n")
         ^ (Printf.sprintf "\tsub r15, 1\n")
         ^ (Printf.sprintf "\tcmp r15, 0\n")
         ^ (Printf.sprintf "\tjne %s\n" label_loop)
         ^ (Printf.sprintf "\t%s:\n" label_loop_exit)
         ^ (Printf.sprintf "\tmov qword[rsp + 8*2 + 8*%d], rax\n"((List.length params') + 1))
         ^ (Printf.sprintf "\tmov r15, %d\n" ((List.length params') + 1))
         ^ (Printf.sprintf "\tmov qword[rsp + 8*2], r15\n")
         ^ (Printf.sprintf "\tmov r10, rsp\n")
         ^ (Printf.sprintf "\tadd r10, 8*2 + 8*%d\n" ((List.length params') + 1))
         ^ (Printf.sprintf "\tsub r9, [rsp + 8*2]\n")
         ^ (Printf.sprintf "\tmov r15, r9\n")
         ^ (Printf.sprintf "\tshl r15, 3\n")
         ^ (Printf.sprintf "\tmov rbx, r15\n")
         ^ (Printf.sprintf "\tadd r15, r10\n")
         ^ (Printf.sprintf "\tmov r11, [rsp + 8*2]\n")
         ^ (Printf.sprintf "\tadd r11, 3\n")
         ^ (Printf.sprintf "\t%s:\n" label_loop_params3)
         ^ (Printf.sprintf "\tmov r12, [r10]\n")
         ^ (Printf.sprintf "\tmov [r15], r12\n")
         ^ (Printf.sprintf "\tsub r10, 8\n")
         ^ (Printf.sprintf "\tsub r15, 8\n")
         ^ (Printf.sprintf "\tsub r11, 1\n")
         ^ (Printf.sprintf "\tcmp r11, 0\n")
         ^ (Printf.sprintf "\tjne %s\n" label_loop_params3)
         ^ (Printf.sprintf "\t%s:\n" label_loop_params_end3)
         ^ (Printf.sprintf "\tadd rsp, rbx\n")
         ^ (Printf.sprintf "\t%s:\n" label_stack_ok)
         ^ (Printf.sprintf "\tmov r11, qword[rbp]\n")
         ^ (Printf.sprintf "\tenter 0, 0\n")
         ^ (run ((List.length params')+1) (env + 1) body) 
         ^ (Printf.sprintf "\tleave\n")
         ^ (Printf.sprintf "\tmov r11, qword[rbp]\n")
         ^ (Printf.sprintf "\tret 8*3 + 8*%d\n" (List.length params'))
         ^ (Printf.sprintf "\t%s:\n" label_end) 
